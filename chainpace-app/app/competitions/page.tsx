    ...
    <AppShell>
      ...
      <Panel className="mb-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
          <select value={opponent} onChange={(e) => setOpponent(e.target.value)} className="min-w-0 w-full flex-1 rounded-lg border border-border bg-bg px-3.5 py-2.5 text-sm dark:border-border-dark dark:bg-bg-dark sm:min-w-[200px]">
            <option value="">Select a friend</option>
            {chain.friends.map((f) => (
              <option key={f} value={f}>{shortAddr(f)}</option>
            ))}
          </select>
          <input type="number" min={1} max={30} value={days} onChange={(e) => setDays(Number(e.target.value))} className="w-full rounded-lg border border-border bg-bg px-3 py-2.5 text-sm dark:border-border-dark dark:bg-bg-dark sm:w-24" />
          <button
            onClick={async () => {
              setBusy(true);
              try {
            ...
            disabled={!opponent || busy || !chain.ready}
          ...
          return (
            <Panel key={c.id} className="!border-violet-dark/30 !bg-gradient-to-br !from-violet-dark/12 !to-mint/5">
              <div className="mb-3 flex items-center justify-between gap-2">
                ...
                <Chip tone="coral">{left}d left</Chip>
              </div>
              <div className="my-3 flex items-center justify-between">
                <div className="flex flex-1 flex-col items-center gap-1"><Avatar initials="YOU" size={40} /><div className="font-display text-xl font-bold">{c.youScore}</div></div>
                <div className="font-mono text-[11px] text-faint">VS</div>
                <div className="flex flex-1 flex-col items-center gap-1"><Avatar initials={initialsFromAddr(other)} size={40} /><div className="font-display text-xl font-bold">{c.themScore}</div></div>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-surface2">
                <div className="h-full bg-violet-bright" style={{ width: `${(c.youScore / total) * 100}%` }} />
              </div>
              {now > c.endTime && (
                <button onClick={() => chain.resolveCompetition(c.id)} className="mt-3 w-full rounded-lg bg-violet-dark py-2 text-[12px] font-semibold text-white">Resolve</button>
              )}
            </Panel>
          );
        })}
