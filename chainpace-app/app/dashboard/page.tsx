        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.6fr_1fr]">
          <Panel>
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link href="/habits" className="text-xs font-semibold text-violet-bright">
                View all
              </Link>
            </div>
            {chain.habits.length === 0 ? (
              ...
              chain.habits.map((h) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 text-[13.5px] font-medium">{h.title}</div>
                  <div className="font-mono text-[11px] text-gold">🔥 {h.streak}</div>
                </div>
              ))
            )}
          </Panel>
            ...
            <Panel>
              ...
              {!activeComp ? (
                ...
                <p className="text-sm">
                  vs {shortAddr(
                    chain.address &&
                      activeComp.challenger.toLowerCase() === chain.address.toLowerCase()
                      ? activeComp.opponent
              ...
              <Link href="/competitions" className="mt-3 inline-block text-xs font-semibold text-violet-bright">
                Open competitions
              </Link>
            </Panel>
            <Panel>
              ...
              {[...chain.circle]
                ...
                .slice(0, 5)
                .map((m, i) => (
                  <div key={m.address} className="flex items-center justify-between py-1.5 text-[13px]">
                    <span>
                      {i + 1}.{" "}
                      {chain.address && m.address.toLowerCase() === chain.address.toLowerCase()
                        ? "You"
                        : shortAddr(m.address)}
                    </span>
              ...
              {chain.circle.length === 0 && (
                <p className="text-sm text-faint">Connect wallet to load scores.</p>
              )}
            </Panel>
            <Panel>
              <h3 className="mb-2 font-display text-[15px] font-semibold">Rewards</h3>
              <p className="text-sm">{chain.points} pts on-chain</p>
              {chain.has14 && <p className="text-[13px]">🏅 14-day badge</p>}
              {chain.has30 && <p className="text-[13px]">🏆 30-day badge</p>}
            </Panel>
          </div>
        </div>
      </main>
