...
export default function DashboardPage() {
  ...
  const name =
    user?.username || (chain.address ? shortAddr(chain.address) : "there");
  const activeComp = chain.competitions.find((c) => !c.resolved);
  const switchNetwork = async (chainIdHex: string | number) => {
    ...
    try {
      ...
      if (error.code === 4902) {
        // Network not added, add it manually
        await window.ethereum.request({
    ...
    <AppShell>
      ...
      {chain.wrongNetwork && (
        <p className="mb-4 text-sm text-coral">
          <button
            onClick={() => switchNetwork("0x530")}
            className="text-gold animate-pulse-glow underline"
          >
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Stat
          label="Circle rank"
          value={`#${chain.rank} of ${chain.circleSize}`}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.6fr_1fr]">
        <Panel>
          <div className="mb-4 flex items-center justify-between gap-2">
            ...
            <Link
              href="/habits"
              className="text-xs font-semibold text-violet-bright"
            >
              View all
            </Link>
          </div>
          {chain.habits.length === 0 ? (
            ...
            chain.habits.map((h) => (
              ...
              >
                ...
                <div className="shrink-0 font-mono text-[11px] text-gold">
                  🔥 {h.streak}
                </div>
              </div>
            ))
          ...
          <Panel>
            ...
            {!activeComp ? (
              <p className="text-sm text-faint">
                None yet. Challenge a friend on-chain.
              </p>
            ) : (
              <p className="break-all text-sm">
                ...
                {shortAddr(
                  chain.address &&
                    activeComp.challenger.toLowerCase() ===
            ...
            <Link
              href="/competitions"
              className="mt-3 inline-block text-xs font-semibold text-violet-bright"
            >
              Open competitions
            </Link>
          </Panel>
              ...
              .map((m, i) => (
                ...
                >
                  <span className="min-w-0 truncate">
                    {i + 1}.{" "}
                    {chain.address &&
                    m.address.toLowerCase() === chain.address.toLowerCase()
                      ? "You"
                      : shortAddr(m.address)}
                  </span>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <Panel>
      ...
      <div className="mt-1 break-words font-display text-lg font-semibold sm:text-[22px]">
        {value}
      </div>
    </Panel>
  );
