    ...
    <AppShell>
      ...
      {chain.wrongNetwork && (
        <p className="mb-4 text-sm text-coral">
          <button onClick={() => switchNetwork("0x530")} className="text-gold animate-pulse-glow underline">
      ...
      <MindsetPanel habits={chain.habits.map((h) => ({ title: h.title, streak: h.streak }))} />
            ...
            chain.habits.map((h) => (
              <div key={h.id} className="flex items-center gap-3 border-b border-bordersoft py-2.5 last:border-none dark:border-bordersoft-dark">
                <button
                  onClick={() => {
                    if (h.done) return;
                    chain.logCompletion(h.id);
                    agentEvent({ habitName: h.title, status: "completed" }).catch(() => {});
                  }}
              ...
              .map((m, i) => (
                <div key={m.address} className="flex items-center justify-between gap-2 py-1.5 text-[13px]">
                  <span className="min-w-0 truncate">
                    {i + 1}.{" "}
                    {chain.address && m.address.toLowerCase() === chain.address.toLowerCase() ? "You" : shortAddr(m.address)}
                  </span>
