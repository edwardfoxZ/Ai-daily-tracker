...
export default function ProfilePage() {
  ...
  if (loading) {
    return <AppShell><p className="text-sm text-faint">Loading profile…</p></AppShell>;
  }
  if (!user) {
    return (
      <AppShell>
  ...
  return (
    <AppShell>
      ...
      {walletErr && <div className="mb-4 rounded-xl border border-coral/30 bg-coral/10 px-4 py-3 text-sm text-coral">{walletErr}</div>}
      {walletMsg && <div className="mb-4 rounded-xl border border-mint/30 bg-mint/10 px-4 py-3 text-sm">{walletMsg}</div>}
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_1.4fr]">
        <div className="flex flex-col gap-5">
          <Panel>
            <div className="flex flex-col items-center text-center">
              <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-bright to-violet-deep font-display text-2xl font-bold text-white">{initialsFrom(user.username)}</div>
              {editingUsername ? (
              ...
              <div className="mt-5 grid w-full grid-cols-3 gap-2 border-t border-bordersoft pt-4 dark:border-bordersoft-dark">
                <div className="text-center"><div className="font-display text-lg font-bold">{chain.habits.filter((h) => h.active).length}</div><div className="text-[10px] text-faint">Active habits</div></div>
                <div className="text-center"><div className="font-display text-lg font-bold">{chain.points}</div><div className="text-[10px] text-faint">On-chain pts</div></div>
                <div className="text-center"><div className="font-display text-lg font-bold">{chain.friends.length}</div><div className="text-[10px] text-faint">Friends</div></div>
              </div>
            </div>
          </Panel>
          <Panel>
            <PanelHead title="Session" />
            <button onClick={fullLogout} className="w-full rounded-lg border border-coral/30 bg-coral/10 py-2.5 text-[13px] font-semibold text-coral">Log out and disconnect wallet</button>
          </Panel>
        </div>
        <div className="flex flex-col gap-5">
          <Panel>
          ...
          <Panel>
            <PanelHead title="Wallet" sub={liveAddress ? "Connected on this device" : "Connect with WalletConnect"} />
            {liveAddress ? (
